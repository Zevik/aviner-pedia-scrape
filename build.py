...
def parse_filename(filename):
    ...
    topic_patterns = {
        ...
        'יורה דעה': r'יורה\s+דעה',
        'חושן משפט': r'חושן\s+משפט',
        'עניינים שונים': r'עניינים\s+שונים',
        'שו"ת לפי נושא': r'שו"?ת.*נושא',
        'אקטואליה': r'אהבת\s+החינם|אלאור\s+עזריה|אנטישמיות|בג.*ץ.*עפרה|חוק\s+ההסדרה|בחירות|ביטוח|עמונה|בוררות|מאבקים\s+ציבוריים|בשר\s+מתורבת|בתי\s+מדרש\s+חילוניים|גיוס\s+חרדים|גיור\s+פרטי|סוף\s+העולם|האפיפיור|חיילים\s+שפינו|סופה\s+סנדי|הסכם.*טורקיה|הסכמים.*ארץ\s+ישראל|הספד.*עובדיה\s+יוסף|הפוליטיקאי|מפלגה\s+חילונית|הכרוז.*הבית\s+היהודי|הרבנות\s+הראשית|האפיפיור|הרפורמה|השכרת.*ערבים|התפקדות|זהירות.*בחירות|זכויות.*פועלים|חולה\s+פוליו|חוק\s+גיוס|חוק\s+הגיור|חטיפת\s+הנערים|חיסון|מגנומטרים.*פוליטיקה|חקיקת\s+חוקים|טיפול.*מחבלים|טרור|כפר\s+דומא|כלכלה\s+סוציאליסטית|כסף\s+מזויף|למה\s+אין\s+שלום|למי\s+להצביע|לשון\s+הרע.*בחירות|מאפיונרים|עמוד\s+ענן|נביאי\s+שקר|נשים\s+בוחרות|סדר\s+יום\s+יהודי|סוריה|סכנה.*העיר\s+העתיקה|סמארטפון|ספרות|ספרי\s+פנטזיה|עימותים.*דתיים.*חילונים|וירוס.*קורונה|עליה\s+להר\s+הבית|עליית\s+הפלשמורה|ערבים\s+מעצבנים|ערבים\s+מתנכלים|עצרת.*שחרור|פולארד|פוליטיקה.*רבנים|פיגועים|פינוי.*בית\s+כנסת|מגרון|פלאפון\s+כשר|פסטיבל|פרסומות.*בעלונים|צוק\s+איתן|קייטנות.*הקרן\s+לידידות|רב\s+ראשי|רבנות\s+ראשית|רפורמה|רפורמים|שריפות|שויון\s+בנטל|שיוויון\s+בנטל|שירות\s+צבאי.*נשים|שמירת\s+הלשון.*ציבוריים|תאונות\s+דרכים|תנ.*ך\s+בגובה\s+העיניים',
    }
